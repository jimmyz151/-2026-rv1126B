...
拍照 + 检测 分离模式
  - 屏幕实时显示摄像头
  - 按 s: 保存当前帧 (不跑NPU)
  - 按 a: 关闭摄像头 → NPU批量检测所有已保存图片 → 串口输出 → 退出
"""
import os
os.environ.setdefault("GST_V4L2SRC_DEFAULT_DEVICE", "/dev/video-camera0")

import cv2, numpy as np, time, sys, fcntl, glob
...
def main():
    ...
    if not cap.isOpened(): print("cam fail"); return
    ...
    while not stage1_done:
        ...
        if key == 'a':
            ...
            cv2.waitKey(200)
            print(f"  [拍照] {path} (共 {len(saved)} 张)")

        fc += 1
        if time.time() - t0 >= 1.0:
            fps = fc / (time.time() - t0); fc = 0; t0 = time.time()
        cv2.putText(frame, f"FPS:{fps:.1f} | s:save({len(saved)}) a:detect", (4, 14),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 1)
        cv2.imshow("CAM", frame)

    # ── 阶段2: 关摄像头, NPU 批量检测 ──
    print(f"\n{'='*50}")
    print(f"  阶段2: NPU 批量检测 ({len(saved)} 张)")
    print(f"{'='*50}")
    ...
    for i, path in enumerate(saved):
        img = cv2.imread(path)
        if img is None:
            print(f"  [{i+1}/{len(saved)}] {path} 读取失败")
            continue
    ...
    rknn.release()
    print(f"\n{'='*50}")
    print(f"  完成! {len(saved)} 张图, 共 {total_results} 个缺陷")
